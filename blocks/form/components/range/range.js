...
function updateBubble(input, wrapper) {
  ...
  const left = sliderWidth
    ? ((sliderWidth - thumbWidth) * percent) / 100 + (thumbWidth / 2)
    : `${percent}%`;

  bubble.style.left = typeof left === 'number' ? `${left}px` : left;
  bubble.style.transform = 'translateX(-50%)';

  if (customThumb) {
    customThumb.style.left = typeof left === 'number' ? `${left}px` : left;
  }
...
export default async function decorate(fieldDiv, fieldJson) {
  ...
  // Show default bubble/thumb immediately on page load
  requestAnimationFrame(() => {
    showSliderValue(wrapper);
    updateBubble(input, wrapper);

    // Trigger EMI/card update on initial load
    input.dispatchEvent(new Event('input', { bubbles: true }));
    input.dispatchEvent(new Event('change', { bubbles: true }));
  });
