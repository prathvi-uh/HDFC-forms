...
function calculateEMI(globals) {
  ...
  function getActualValueFromSlider(value, ticks) {
    value = Number(value);
    ...
    // already actual value
    if (value > maxIndex) return value;
  ...
  const loanRaw = Number(globals.form.offer.loanamt.valueOf());
  const tenureRaw = Number(globals.form.offer.loantenure.valueOf());

  const savedLoanRaw = loanRaw > 0 ? loanRaw : Number(existing.loanRaw || 6);
  const savedTenureRaw = tenureRaw > 0 ? tenureRaw : Number(existing.tenureRaw || 6);
